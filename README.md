...
本项目是软件工程实训任务二的小组协同开发项目，基于 `world-of-zuul` 文字冒险样例扩展为前后端分离的可视化冒险游戏。项目围绕房间探索、物品拾取、背包负重、传送房间、用户登录注册、存档读档、能力升级和图形化界面等功能进行迭代，并通过 GitHub Actions 验证后端测试、后端打包和前端构建。

## 功能概览

- 前后端分离架构：后端提供 Spring Boot API，前端提供 Vue 图形化界面。
- 地图与房间系统：支持多个房间、方向移动、楼层移动、返回上一房间和传送房间。
- 玩家与背包系统：玩家可拾取、丢弃、查看物品，背包受重量上限约束，并提供底部物品栏和背包详情浮层。
- 理智增强剂机制：吃掉特定物品后可提升玩家最大负重。
- 用户与存档系统：支持登录注册，并通过数据库保存和读取游戏状态。
- 游戏化前端界面：支持俯视角房间网格、自由移动、角色动画、房间地图、帮助文档、操作日志和背景音乐。
...
- `master`：最终稳定发布分支，通过发布快照同步当前可验收项目版本。
...
当前稳定版本按 `release/master-snapshot-*` 发布快照方式从 `feature` 同步到 `master`。课程验收结束前不建议删除 `feature` 或历史工作分支，用于保留协同开发过程证据；验收结束后再统一清理临时分支。
